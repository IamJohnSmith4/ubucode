...
# ==================================================
# NODE PATH SEQUENCES
# FORWARD  (target > current): sequence from Node 1
# BACKWARD (target < current): direct only
# (1→2) and (1→3) use hardcoded moves first, then nav
# ==================================================
NODE_PATH_SEQUENCES = {
    1: [1],
    2: [1, 2],
...
# Routes that start with hardcoded moves before switching to nav mode.
# Format: list of ("rotate", deg) or ("move", dist, bias) commands.
# After these finish, the robot hands off to move_base for the rest.
HARDCODED_PREFIX = {
    (1, 2): [("rotate", -90), ("move", 6.5, 0.0), ("rotate", 90)],
    (1, 3): [("rotate", -90), ("move", 6.5, 0.0), ("rotate", 90)],
}
...
class OdomRobot:
    ...
    def execute_home_sequence(self):
        ...
        current_location = 1
        is_navigating = False
        rospy.loginfo("--- Home Sequence Complete: at Node 1 ---")

    # --------------------------------------------------
    # MOTION PRIMITIVES  (used by home sequence &
    ...
    def move_forward(self, distance, bias=0.0):
        ...
        rospy.loginfo(f"[Hardcode] move_forward {distance:.2f}m  bias={bias}")
    ...
    def rotate(self, angle_rad):
        ...
        rospy.loginfo(f"[Hardcode] rotate {math.degrees(angle_rad):.1f}°")
    ...
    def _run_hardcoded_prefix(self, cmds):
        """Execute a list of hardcoded move/rotate commands."""
        for cmd in cmds:
            ...
            if action == "move":
                ...
                self.rotate(math.radians(float(cmd[1])))
                rospy.sleep(0.5)
        return True
    ...
    #   If route has a HARDCODED_PREFIX → run those moves
    #   first (odom-based), then hand off to move_base
    #   nav for remaining waypoints.
    #   Otherwise → pure move_base nav through sequence.
    #
    # BACKWARD (target < current):
    #   Direct move_base nav to target only.
    #
    # Routes with hardcoded prefix:
    #   (1→2): rotate -90 → move 6.5m → rotate 90  → nav to Node 2
    #   (1→3): rotate -90 → move 6.5m → rotate 90  → nav to Node 2 → nav to Node 3
    # --------------------------------------------------
    def execute_path(self, start, target):
        ...
        # ── FORWARD ──
        full_sequence = NODE_PATH_SEQUENCES.get(target, [target])
        prefix_key    = (current_location, target)
        prefix_cmds   = HARDCODED_PREFIX.get(prefix_key)

        if prefix_cmds:
            # --- Phase 1: hardcoded moves ---
            rospy.loginfo(
                f"[Path] FORWARD {current_location} → {target} | "
                f"Phase 1: hardcoded moves {prefix_key}"
            )
            ok = self._run_hardcoded_prefix(prefix_cmds)
            if not ok:
                rospy.logwarn("[Path] Hardcoded prefix aborted.")
                return False
            rospy.loginfo("[Path] Phase 1 complete — handing off to nav mode.")

            # --- Phase 2: nav mode for remaining nodes after current_location ---
            # After the hardcoded moves the robot is physically past Node 1,
            # so we navigate only the nodes that come AFTER it in the sequence.
            try:
                start_idx = full_sequence.index(current_location) + 1
            except ValueError:
                start_idx = 0
            remaining = full_sequence[start_idx:]

        else:
            # --- Pure nav mode (no hardcoded prefix) ---
            rospy.loginfo(
                f"[Path] FORWARD {current_location} → {target} | "
                f"pure nav sequence: {full_sequence}"
            )
            remaining = full_sequence

        total_steps = len(remaining)
        for step_idx, waypoint in enumerate(remaining, start=1):
            rospy.loginfo(
                f"[Path] Step {step_idx}/{total_steps} — nav to Node {waypoint}"
            )
...
def handle_reset_home():
    ...
    return jsonify({
        "status":  "starting",
        "message": "Executing home sequence → Node 1",
    }), 200
