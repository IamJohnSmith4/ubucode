...
# --- คลาส PID พารามิเตอร์ตามภาพ 4.2 ---
class PID:
...
class RobotLogger:
    def __init__(self):
        ...
        # ตั้งค่า PID (Kp=1.8, Ki=0.005, Kd=0.1)
    ...
    def move_forward(self, distance, bias=0.0):
        ...
        # รอให้ระบบนิ่ง 1 วินาที แทนการใช้ while loop ที่อาจทำให้ค้าง
        rospy.sleep(1.0)
        
        # 2. ตั้งจุดเริ่มต้น ณ ตำแหน่งปัจจุบัน (Relative Origin) 
        # วิธีนี้จะแก้ปัญหาหุ่นไม่วิ่งถ้า x, y ไม่เป็น 0
        ...
        accel = 0.01 # เพิ่มอัตราเร่งเล็กน้อยเพื่อให้หุ่นเริ่มขยับง่ายขึ้น
        min_speed = 0.08
        decel_dist = 0.45
        ...
        rospy.loginfo(f"ระบบ: เริ่มเดินหน้า {distance} ม. จากจุด ({start_x:.2f}, {start_y:.2f})")
        ...
        while not rospy.is_shutdown() and self.is_navigating:
            ...
            # คำนวณ Yaw Error เพื่อให้หุ่นเดินตรง
    ...
    def save_results(self, target_dist):
        # บันทึก CSV (UTF-8 with BOM สำหรับ Excel ภาษาไทย)
        csv_file = "ผลการทดสอบ_เดินตรง.csv"
        ...
        plt.subplot(2, 1, 1)
        plt.plot(t_vals, s_vals, 'b', label='Speed (m/s)')
        plt.title('Robot Movement Profile')
        plt.grid(True); plt.legend()

        plt.subplot(2, 1, 2)
        plt.plot(t_vals, d_vals, 'g', label='Distance (m)')
        plt.xlabel('Time (s)'); plt.grid(True); plt.legend()

        plt.savefig('graph_output.png')
        rospy.loginfo(f"บันทึกไฟล์ '{csv_file}' และ 'graph_output.png' สำเร็จ")
